...
export async function GET(request: NextRequest) {
  try {
    ...
    let allSettings = [];
    try {
      allSettings = await settingsAPI.getAll(user.id);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'sharetribe') {
      ...
      // Save sharetribe settings
      await settingsAPI.setMultiple(user.id, 'sharetribe', {
