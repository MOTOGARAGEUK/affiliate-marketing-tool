  ...
  try {
    ...
    const payouts = affiliates?.map(affiliate => {
      ...
      return {
        ...
        amount: Math.round(totalEarnings * 100) / 100, // Round to 2 decimal places
        totalReferrals,
        ...
        status: totalEarnings > 0 ? 'pending' : 'no_earnings',
    ...
    // Filter out affiliates with no earnings
    const payoutsWithEarnings = payouts.filter(payout => payout.amount > 0);
